...
// 元素掛在 pokemon-container 裡（跟水面倒影同一套做法）：左右位置、隱藏、
// 移除都跟著本體走。但「離地高度」不跟——那一段寫在 sprite 自己的 transform
// 裡，所以 updateDOM 每幀會另外把它接到名牌上（見那裡的 overlayLift）。
// 這裡原本寫的是「位置都跟著本體走，不必另外照顧」，結果抓起來拖到半空時
// 名牌留在原地、身體直接穿過自己的名字，0.46.1 才補上。
//
