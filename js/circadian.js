...
// 作息 (Circadian)
// ---------------------------------------------------------
// 白天精神好、深夜想睡。過了午夜大家慢下來、站著不動的時間變長、
// 冒出來的多半是 Zzz；天亮之後恢復原本的活潑。
//
// 「幾點算想睡」不自己另定一套，讀 js/sun.js 的 deepNightLevel()——
// 那是「深夜窗口」那一條曲線，夜行陣容（js/roster.js）讀的也是它。
// 刻意「不是」天色那一條（nightLevel，夜晚演出在用）：天黑與想睡不是同一件事，
// 現代人是夜貓子，睡前那幾個小時的活動力跟白天差不多。所以：
//   * 預設窗口 00:00 ~ 06:00（deepStart / deepEnd），18:01 ~ 23:59 完全照白天跑
//   * 午夜「之後」才慢慢睡（nightFade 分鐘的斜坡），不是 00:00 一到就集體倒下
//   * 釘死時刻（?sunTime=03:00）也照樣生效，方便驗收
//   * 想回到「天一黑就睡」就把窗口設成日落日出：?deepStart=18&deepEnd=6
//
// 白天 = config.js 寫的那組數字，一個都不動；只有深夜才乘上下面的倍率。
...
// 現在有多想睡：0 = 白天與睡前那幾個小時（完全照 config.js）、1 = 深夜。
// nightSleep 是總強度，0 就整套關閉（深夜也跟白天一樣活潑）
function sleepiness() {
    const strength = Math.min(Math.max(CONFIG.nightSleep ?? 1, 0), 1);
    if (strength <= 0) return 0;
    return deepNightLevel() * strength;
}
