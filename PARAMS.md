...
| `nightSleep` | float | 0 ~ 1 | `1` | **夜晚作息**:天黑之後大家慢下來、站著不動的時間變長、冒出來的多半是 **Zzz**;天亮之後恢復原本的活潑。這個值是「有多想睡」的總強度,`0` = 整套關閉(晚上跟白天一樣活潑)。**白天完全照 `config.js` 的數字跑**,只有夜裡才乘上倍率。詳見下方「夜晚作息」 |
| `nightRoster` | float | 0 ~ 1 | `0.75` | **夜行陣容**:夜間偏抽「夜行系」(幽靈 + 惡)的機率,每個名額各擲一次骰。**只認主屬性**(與 `team` 刻意不同,理由見下)。詳見下方「主題隊伍與夜行陣容」。`0` = 不偏抽 |
...
| 完全不要夜晚演出 | `?night=off` |
| **看夜晚作息**(深夜:大半站著睡,走的也慢) | `?sunTime=23:00&count=6` |
| 黃昏睡到一半(睡意約 0.44) | `?sunTime=18:20&nightFade=45` |
| 晚上跟白天一樣活潑(關掉作息) | `?nightSleep=0` |
...
夜晚演出改的是**畫面**,這一節改的是**行為**:天黑之後牠們真的想睡。
...
**幾點算晚上**跟夜晚演出同一條線:預設日落 18:00 之後慢慢睡、日出 06:00 前醒,
中間吃 `nightFade` 的斜坡——**不是 18:00 一到就集體倒下**,而是黃昏時越來越慢、
越來越常站著發呆。`?sunTime=23:00` 可以直接跳到深夜驗收。

```
?sunTime=23:00&count=6            深夜:大半站著睡，走的也慢
?sunTime=18:20&nightFade=45       黃昏睡到一半（睡意約 0.44）
?nightSleep=0                     關掉：晚上跟白天一樣活潑
```
