...
| `team` | enum 清單 | normal / fire / water / electric / grass / ice / fighting / poison / ground / flying / psychic / bug / rock / ghost / dragon / dark / steel / fairy | — | **主題隊伍**:只抽這個屬性的寶可夢,十八種屬性的英文名都可以打。也可以逗號並列多種(`team=ghost,dark`)。詳見下方「主題隊伍與夜行陣容」 |
...
| `nightStars` | int | 0 ~ 400 | `70` | 星星數量。撒在畫面上半部(下面留給地面與散步的舞台),每顆有自己的亮度、閃爍週期與相位,所以不會齊步閃。`0` = 不要星空 |
| `nightFireflies` | int | 0 ~ 120 | `12` | 螢火蟲數量。貼近地面緩緩游移的光點,一邊飄一邊明滅(明滅週期刻意跟飄移錯開,才不會每次都在同一點亮起來)。`0` = 不要螢火蟲 |
| `nightGlow` | float | 0 ~ 1 | `0.35` | **地面光暈**濃度:貼著地面往上暈開的一層微光,最濃處在地面表面,上緣永遠收乾成全透明。光是「積在地面上」的,所以 `theme=none` 時自動不生效。`0` = 不要光暈 |
| `nightFade` | int | 0 ~ 180 | `45` | 日落後漸暗(日出前漸亮)要花幾分鐘。天文上的暮光也是日落**之後**才開始,所以日落那一刻夜色還是 0。`0` = 不漸變,日落瞬間直接入夜 |
| `nightRoster` | float | 0 ~ 1 | `0.75` | **夜行陣容**:夜間偏抽「夜行系」(幽靈 + 惡)的機率,每個名額各擲一次骰。詳見下方「主題隊伍與夜行陣容」。`0` = 不偏抽 |
...
| 夜行陣容(幽靈+惡,不必等天黑) | `?team=ghost,dark&count=6` |
...
| 整晚都是幽靈(夜行偏好開到滿) | `?sunTime=23:00&nightRoster=1&count=6` |
| 晚上也照常隨機(關掉夜行偏好) | `?nightRoster=0` |
| 完全不要夜晚演出 | `?night=off` |
...
> **只看主屬性。** 屬性查的是 `pokemon_types.js`,那張表只存**主屬性**(slot 1),
> 副屬性算不進來。多數屬性沒差,但**飛行系特別明顯**:全 1025 隻裡主屬性是
> `flying` 的只有 9 隻,預設範圍(`maxId=649`)內更是只有 1 隻——大比鳥
> (一般/飛行)這種「副屬性才是飛行」的都不算。這是資料的邊界,不是壞掉。
...
預設 `0.75` = 晚上大約四隻裡有三隻是這一味。刻意**逐個名額擲骰**而不是整批換掉:
偶爾混進一隻不是夜行系的,那一隻反而讓整批看起來是抽出來的,而不是排好的。
想更徹底就 `?nightRoster=1`,不要就 `?nightRoster=0`。

判定時刻用「夜色過半」當門檻(所以 `nightFade` 也會挪動它)。陣容只在**生成的
...
`?night=off&nightRoster=1` 就是「畫面照常,但晚上都是幽靈」。
