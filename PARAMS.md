...
| `drag` | enum | `on` / `off` | `on` | **滑鼠拖曳**總開關:快速點一下 = 戳戳互動;**按住**超過 `dragHoldTime` 就把那一隻「抓起來」——跟著游標走、在手上掙扎,放開手才自由落體回地面。抓在手上的那隻等於暫時離場:不散步、不冒對話框、不寒暄、戳不動,果實也不會指派給牠(點擊、遙控 `feed`、信使鳥空投三個入口一致——寧可整顆叼走)。游標拉出活動範圍時牠**貼著邊界繼續掙扎**,不會被拖出場外。`off` = 只保留點一下的戳戳互動 |
| `dragHoldTime` | int | 0 ~ 2000 | `200` | 長按門檻 ms:按住超過這個時間才算「抓起來」,之內放開就是單純點一下。調小 = 一按就黏手,調大 = 要按久一點才抓得起來 |
...
| 一抓就黏手(拖曳門檻放到最低、掙扎更激烈) | `?dragHoldTime=0&dragStruggleRate=4` |
| 純觀賞(不給抓,只留戳戳互動) | `?drag=off` |
...
| `poke` | `id`(選填) | 開心跳一下 + 冒愛心(跟滑鼠點擊同一種互動)。帶 `id` 只戳該圖鑑編號的成員,不帶就全員;**正被滑鼠抓在手上的戳不動**,回執的 `count` 也不算牠 |
